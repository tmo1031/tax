...
export function getDisabilityDeduction(taxYear, disabilityLevel) {
  ...
  const DisabilityDeductionTable =
    taxYear > 1982
      ? {
...
export function getSpouseDeduction(taxYear, hasSpouse, applicantTotal, spouseTotal, spouseAge) {
  ...
  function shrinkDeductions(deductions, rate, mode) {
    let shrinkValue =
      mode === 'ceil'
        ? Math.ceil((deductions / 30000) * rate) * 10000
        : mode === 'round'
        ? Math.round((deductions / 30000) * rate) * 10000
        : deductions;
    return shrinkValue;
  }
...
export function getDividendCredit(taxYear) {
  // 配当控除の計算が難しいので保留
  return { incomeTax: 0, residentTax: 0 };
}
...
export function getLoansCreditPre(estate, taxable) {
  ...
  let LoanTable;
  if (MoveInYear < 1999) {
    LoanTable = [{ age: null, max: null, rate: null, incomeLimit: null }];
